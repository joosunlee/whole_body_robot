...
private: 
  ...
  Eigen::VectorXd Force_x = Eigen::VectorXd::Zero(1);
  // double Force_x = 0;
  ...
  Eigen::VectorXd Admittance = Eigen::VectorXd::Zero(4);
...
public:
  AdmittanceController()
  {
    ...
    joint_state_sub = n_.subscribe("/mobile_robot/joint_states", 1, &AdmittanceController::JointStateCallback, this);
    fl_velocity_pub = n_.advertise<std_msgs::Float64>("/mobile_robot/left_front_arm_controller/command", 5);
    fr_velocity_pub = n_.advertise<std_msgs::Float64>("/mobile_robot/right_front_arm_controller/command", 5);
    rl_velocity_pub = n_.advertise<std_msgs::Float64>("/mobile_robot/left_back_arm_controller/command", 5);
    rr_velocity_pub = n_.advertise<std_msgs::Float64>("/mobile_robot/right_back_arm_controller/command", 5);;
  ...
  void StateCallback(const gazebo_msgs::LinkStates::ConstPtr& state_msg)
  {
    ...
    current_vel << state_msg->twist[1].linear.x, state_msg->twist[1].linear.y, state_msg->twist[1].linear.z, state_msg->twist[1].angular.x, 
                    state_msg->twist[1].angular.y, state_msg->twist[1].angular.z; //mobile_body
...
//step1
      // dthetalist = -(Force_x / D);
//step2
      Admittance = -(Force_x / D)*(1-exp(-t/tau));
      ...
      cout << "mobile velocity1 : " << velocity1 << endl;
      cout << "mobile velocity2 : " << velocity2 << endl;
      cout << "mobile velocity3 : " << velocity3 << endl;
      cout << "mobile velocity4 : " << velocity4 << endl;
